function varargout = maxiGS911(varargin)
% MAXIGS911 MATLAB code for maxiGS911.fig
%      MAXIGS911, by itself, creates a new MAXIGS911 or raises the existing
%      singleton*.
%
%      H = MAXIGS911 returns the handle to a new MAXIGS911 or the handle to
%      the existing singleton*.
%
%      MAXIGS911('CALLBACK',hObject,eventData,handles,...) calls the local
%      function named CALLBACK in MAXIGS911.M with the given input arguments.
%
%      MAXIGS911('Property','Value',...) creates a new MAXIGS911 or raises the
%      existing singleton*.  Starting from the left, property value pairs are
%      applied to the GUI before maxiGS911_OpeningFcn gets called.  An
%      unrecognized property name or invalid value makes property application
%      stop.  All inputs are passed to maxiGS911_OpeningFcn via varargin.
%
...
gui_State = struct('gui_Name',       mfilename, ...
    ...
    'gui_OpeningFcn', @maxiGS911_OpeningFcn, ...
    'gui_OutputFcn',  @maxiGS911_OutputFcn, ...
    'gui_LayoutFcn',  [] , ...
    'gui_Callback',   []);
...
% --- Executes just before maxiGS911 is made visible.
function maxiGS911_OpeningFcn(hObject, eventdata, handles, varargin) %#ok<*INUSL>
% This function has no output args, see OutputFcn.
% hObject    handle to figure
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    structure with handles and user data (see GUIDATA)
% varargin   command line arguments to maxiGS911 (see VARARGIN)

% Choose default command line output for maxiGS911
handles.output = hObject;

% Update handles structure
guidata(hObject, handles);

% UIWAIT makes maxiGS911 wait for user response (see UIRESUME)
...
function varargout = maxiGS911_OutputFcn(hObject, eventdata, handles)
...
% handles    structure with handles and user data (see GUIDATA)
currFolder=cd;
currfFile=get(handles.txt_file,'String');
[p,~,~]=fileparts(currfFile);
try
    cd(p);
catch
    %the previous cd was not successful - do nothing
end
...
function pb_extractd_Callback(hObject, eventdata, handles) %#ok<DEFNU>
...
if get(handles.cb_load_all_files,'value') %load all csv and txt log files
    [pathstr, ~, ~]= fileparts(get(handles.txt_file,'String')) ;
    
    
    files=dir([pathstr '\*.csv']);
    files=[files; dir([pathstr '\*.txt'])];
    
    fnames=sort({files.name}'); %#ok<TRSRT>
    ...
    for m=1:length(fnames)
        ...
        [data_buf, cols_names_buf]=extract_data([pathstr '\' fname],parameters); %#ok<NASGU>
...
function pb_export_data_Callback(hObject, eventdata, handles) %#ok<DEFNU>
...
function cb_compress_date_scale_Callback(hObject, eventdata, handles) %#ok<DEFNU>
...
function tbl_par_CellSelectionCallback(hObject, eventdata, handles) %#ok<DEFNU>
